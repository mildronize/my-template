...
| BFF framework | gin — reuses the existing framework rather than introducing a second one, serves the API side of the BFF | Luna |
| **CORRECTION 2026-08-13 — the rendered view itself was the wrong artifact** | มายด์'s own words, direct: *"the task TPL-1 is simple, replace backend from node (next.js) to go, react keep the same. (SPA is fine)."* **TPL-1 is a backend swap — my-task's React frontend stays, only its Next.js/Node backend becomes Go.** This milestone's `GET /` (a Go `html/template`-rendered page) is not a minimal interim toward that; it's the wrong thing entirely. Root cause: มายด์'s Go stack list was a statement about the *backend* he wanted, and Clara derived "no React/Tailwind/Radix" from it — an implementation choice read as a scope removal, the same shape of error as milestone-1's original grill mistake, on the same axis, twice more the same night. **What survives unaffected**: login/callback/session handling (`golang.org/x/oauth2`+PKCE, the session cookie, all of it), the entire public API, identity, rotate, seed, smoke, the architecture restructure, and this fix round's blockers A–D + the cookie fix — none of that depends on how the view renders. **What's now known-wrong and superseded, not delivered**: the server-rendered HTML in `internal/transport/bff/view_handler.go`, Done-when 9's specific assertion (proves the *pattern* — real domain data rendered through a real session — even though the artifact it exercises is being replaced), and `GETTING-STARTED.md`'s fork guidance for that view. **Not restarting this work now** — Clara's recommendation (มายด์ to decide): finish and close milestone-2 as-is, treat the React SPA as milestone-3, so "repair what's broken" and "add the real frontend" don't get mixed into one harder-to-verify pass | มายด์ (correction), Clara (owning the misread) |
